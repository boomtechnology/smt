...
function command-init {
    ...
    unset PYTHONPATH
    export PYTHONPATH
    if [[ -e "preinstall-requirements.txt" ]]; then
        $VIRTUAL_BIN/pip install -r preinstall-requirements.txt >> build-init.log 2>&1 \
            || die "Pre-Installation failed. Check build-init.log"
    fi

    $VIRTUAL_BIN/pip install -r requirements.txt >> build-init.log 2>&1 \
